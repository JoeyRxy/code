...
| ------------------------------------ | ---------- | ------------------------------ | ----------- | ------------------------------------------------- | --- |
| 共轭($x^*(t)$或$x^*[n]$)             | $X^*(s^*)$ | $X^*(-\operatorname{j}\omega)$ | $X^*(z^*)$  | $X^*(\operatorname{e}^{-\operatorname{j}\omega})$ |     |
| 对称($x(-t)$或$x[-n]$)对应*双边变换* | $X(-s)$    | $X(-\operatorname{j}\omega)$   | $X(z^{-1})$ | $X(\operatorname{e}^{-\operatorname{j}\omega})$   |     |
> 记Laplace 和 Z变换就行
