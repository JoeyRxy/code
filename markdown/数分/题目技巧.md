...
=&\pm\iint_{D_{uv}}\left(P\dfrac{\partial(y,z)}{\partial(u,v)}+...\right){\rm d}u{\rm d}v\\
...
$$
\iint_\Sigma P{\rm d}y{\rm d}z+Q{\rm d}x{\rm d}z+R{\rm d}x{\rm d}y\\=\iint_\Sigma (P,Q,R)\cdot(\cos\alpha,\cos\beta,\cos\gamma){\rm d}S
$$
## 第二型曲线积分
## 原始方法
直接换参数，很简单
...
\begin{aligned}
  ...
  直接化为一型=&\pm\iint_\Sigma\left|\begin{matrix}
    ...
    P&Q&R
  \end{matrix}\right|dS\\
  直接使用退化版=&\pm\iint_\Sigma\left|\begin{matrix}
    -z_x'&-z_y'&1\\
    \partial_x&\partial_y&\partial_z\\
    P&Q&R
  \end{matrix}\right|dS
