 ...
 * {@link SuffixArray}. The <em>index</em> and <em>length</em> operations takes
 * constant time in the worst case. The <em>lcp</em> operation takes time
 ...
 * <p>
 * In practice, this algorithm runs very fast. However, in the worst-case it can
 * be very poor (e.g., a string consisting of N copies of the same character. We
 * do not shuffle the array of suffixes before sorting because shuffling is
 * relatively expensive and a pathologial input for which the suffixes start out
 * in a bad order (e.g., sorted) is likely to be a bad input for this algorithm
 * with or without the shuffle.
 * <p>
